...
:root{
  ...
  /* 덧쓰기 글씨. 보조선(--fl-aux)과 밝기가 같으면(대비 1.04:1) 연필이 얹히는 순간
     구분이 안 된다. 선보다 한 단계 진하게 둔다. */
  --ghost:#b9c0bf;
  /* 4선지: 보조선은 베이지그레이 한 색으로, 글자를 앉히는 기준선만 민트 굵게 */
  --fl-aux:#d6d1c9; --fl-base:var(--teal);
}
...
/* 보조선 3개는 같은 베이지그레이 — 색이 섞이면 어디에 맞춰 쓸지 헷갈린다.
   기준선(l3)만 민트로 굵게 해서 "여기에 글자를 앉혀라"가 한눈에 보이게. */
.fl-l1 { top:0; border-top:1px solid var(--fl-aux); }
.fl-l2 { top:33.3%; border-top:1px dashed var(--fl-aux); }
.fl-l3 { top:66.6%; border-top:2.2px solid var(--fl-base); }
.fl-l4 { bottom:0; border-top:1px solid var(--fl-aux); }
