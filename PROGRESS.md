...
**1. Bricks 4800 · 능률 중등 필수 — 인쇄물 8.1pt 결정 대기**
14종 중 이 둘만 8.1pt로 하한에 붙어 있다(나머지는 8.8~9.6pt). 24~30단어인데
`--split-wordlist`를 안 걸어서다. 걸면 9.6pt가 되지만 쪽수가 는다:
- Bricks 4800(30단어/유닛) 2권 275쪽 → 약 305쪽
- 능률 중등 필수(24단어/유닛) 3권 371쪽 → 약 421쪽
사용자 판단 필요 — 8.1pt로 둘지, 쪽수를 늘려 9.6pt로 갈지.
...
- `--split-wordlist N` 단어 목록을 N개씩 나눠 여러 장으로. 글자 13.5px, **여백은 지면마다 자동**.
  **판매용 제본 대비 전용** — `make_print_ready.py`가 93% 축소하므로 인쇄물에서 9pt를 넘기려면
  ...
  길어 16mm, 3900은 짧아 87mm). `_wl_pad()`가 지면별로 계산한다. 글자폭 계수
  (한글 0.92 / 영문 0.41)·행간 1.3은 두 교재 56장 실측으로 맞춘 값이고, **절대 과소평가하지
  않도록**(실측 대비 +4~+93px) 골랐다 — 남는 쪽으로 틀리므로 넘치지 않는다.
  가용 높이 913px(표머리 top~로고 top)·표머리 34px·열 폭도 실측값.
...
| Bricks 3100/4800 | ✅ 점검·수정 완료 | 각 2권. 4800만 `--layout v2`(로고 겹침 해소) |
