...
`초등영어 단어가 된다 1·2권`. **별도 생성기 `build_junior.py`** 를 쓴다.
...
```
python build_junior.py "excel/초등영어 단어가 된다_1.xlsx" --answer --out "output_new/초등영어 단어가 된다"
python build_junior.py "excel/초등영어 단어가 된다_2.xlsx" --answer --out "output_new/초등영어 단어가 된다"
```
1권 135쪽 / 2권 132쪽, 미분류 0. 동의어·반의어는 넣지 않음(사용자 지시).
문장은 `scratch_junior/ex_book*.py` → `build_junior_xlsx.py`로 엑셀 재생성.
...
| 초등영어 단어가 된다 1·2 | ✅ **엑셀 완성** — 문장 422개, 워크북까지 생성 | `초등영어 단어가 된다_1` / `_2` |
