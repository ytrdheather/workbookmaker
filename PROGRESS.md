...
**1. 새 교재 임포트** — `PDFs/` 대기 4종(영숙어 1000 · 능률 고등 기본 · 어원편 · 수능 고난도).
아래 "PDF→엑셀 표준 절차" 참고. 수능 필수는 꼬리말 2종 문제 미해결.

**2. 표지·목차 만들 때 16의 배수 맞추기**
어차피 인쇄되는 백지에 속표지·목차·색인·오답노트를 넣으면 손해가 0이 된다.
16대수 기준 백지: 고난도 13쪽 / 초등 39쪽. (오프셋 인쇄일 때만 해당 —
디지털·소량 인쇄는 4의 배수만 맞으면 되고 지금도 손해 5쪽·3쪽뿐)
※ 고난도 분권은 이미 최소다. `--split` 8~20을 다 돌려봤고 13이 최적(백지 13쪽).
...
~~페이지 번호 없음~~ → **2026-08-01 확인 결과 14종 전부 페이지 번호 있음.** 이 메모는 낡은 것이었다.
다만 **Bricks 2300·3100은 단어 목록 자동 축소가 생기기 전 버전**이라 HTML에 `--wlf`가 없다.
CSS 기본값 12.5px(9.4pt)로 렌더되고 25단어가 한 장에 정상적으로 들어간다.
⚠️ **재생성하면 오히려 11.5px(8.6pt)로 작아진다** — 손대지 말 것.
```
...
- `--split-wordlist N` 단어 목록을 N개씩 나눠 여러 장으로. 글자 13.5px, **여백은 지면마다 자동**.
  **판매용 제본 대비 전용** — `make_print_ready.py`가 93% 축소하므로 인쇄물에서 9pt를 넘기려면
  축소 전 9.7pt(13px) 이상이어야 한다. 30단어를 15+15로 나눠 인쇄물 9.6pt 확보.
  ※ `--fit-wordlist`와 같이 쓰지 말 것(정반대 목적).
  ※ 여백을 한 값으로 박으면 안 된다 — 나눈 뒤 남는 높이가 교재마다 다르다(고난도는 뜻이
  길어 16mm, 3900은 짧아 87mm). `_wl_pad()`가 지면별로 계산한다.
  ⚠️ **추정식은 절대 과소평가하면 안 된다.** 과소평가하면 여백을 과하게 줘서 표가 로고를
  덮는다 — 행간 1.3(두 교재 56장으로만 맞춘 값)으로 Bricks 4800을 뽑았더니 24px 모자라
  1.4mm 겹쳤다. **네 교재 300장**으로 다시 맞춰 행간 **1.45**로 확정(실측 대비 +10~+173px).
  글자폭은 한글 0.92 / 영문 0.41. 가용 높이 `WL_BODY_PX=900` — 실측은 v1 913 / v2·logo-top
  909px이라 둘 다 안전하게 900으로 잡았다. 표머리 34px·열 폭도 실측값.
...
| Bricks 3100 | ✅ 점검·수정 완료 | 2권 |
| Bricks 4800 | ✅ **제본용 완료** | 2권(149·156p). `--layout v2`·merge-choice·**split-wordlist 15**. 인쇄물 9.5pt |
| Bricks 3900 | ✅ **제본용 완료** | 900단어/30유닛 → 2권(132·135p). merge-choice·**split-wordlist 15**. 인쇄물 9.6pt |
...
**페이지번호**: 14종 전부 있음(2026-08-01 전수 확인).
...
- **능률VOCA 중등 필수** (1200단어 / 50유닛, DAY 01~50)
  - ✅ 점검·수정 완료. 입력: `excel/능률VOCA 중등 필수.xlsx`.
    **`--merge-choice --fit-choice --logo-top --split-wordlist 12` + 16유닛/권 → 3권**(139·131·151p, 총 421p. 제본용, 인쇄물 9.5pt).
...
- **Bricks Vocabulary 시리즈 7종** — ✅ 전량 생성 완료(2026-07-23). 10유닛/권 분권(15유닛 책은 자투리 합쳐 단권).
  ...
  | 3900 | 900 / 30 | **2권 · 132·135p** | merge-choice·15DAY/권·**split-wordlist 15**(제본용, 인쇄물 9.6pt) |
  | 4800 | 900 / 30 | **2권 · 149·156p** | `--layout v2` 전용. merge-choice·15DAY/권·**split-wordlist 15**(제본용, 인쇄물 9.5pt) |
